...
function exportComponents() {
  ...
  chrome.storage.local.get(['componentsByHost', 'url'], async (data) => {
    ...
    // To track if any image conversion errors occur
    let imageConversionErrors = false;
    ...
    for (const [index, component] of components.entries()) {
      ...
      // If convertImages is checked, process the component's HTML
      if (convertImages && exportedComponent.html) {
        try {
          exportedComponent.html = await convertImagesToDataURLs(exportedComponent.html);
        } catch (error) {
          console.warn(`Error converting images for component ${exportedComponent.name}:`, error);
          imageConversionErrors = true;
        }
      }

      exportedComponents.push(exportedComponent);
    }
    ...
    downloadJSONFile(jsonContent, 'components.json');

    // If there were image conversion errors, alert the user
    if (imageConversionErrors) {
      alert("Unable to load some images. Has the page changed since selecting your components?");
    }
  });
}

// Function to process images in HTML and convert them to Data URLs
async function convertImagesToDataURLs(htmlString) {
  ...
  const promises = Array.from(imgElements).map(async (img) => {
    ...
    if (src) {
      try {
        ...
        const dataURL = await getImageAsDataURL(src);
        console.log('Converted image to Data URL:', dataURL);
        img.setAttribute('src', dataURL);
      } catch (error) {
        console.warn(`Could not convert image ${src} to Data URL:`, error);
        // Rethrow the error to be caught in the parent function
        throw error;
      }
    }
  });
...
function getImageAsDataURL(src) {
  return new Promise((resolve, reject) => {
    ...
    // Set the source to trigger image loading
    img.src = src;
  });
}
